...
/* ─── Page ────────────────────────────────────────────────── */
export default function AdminAgencyClientDetailPage() {
    ...
    useEffect(() => {
        const fetchClientDetail = async () => {
            if (!params.id) return;
            try {
                const res = await fetch(`/api/admin-agency/clients/${params.id}`);
                ...
                if (res.ok) {
                    ...
                    setError(data.error || "Gagal memuat data klien.");
                }
            } catch (e) {
    ...
    if (error || !client) {
        return (
            <div className="flex flex-col items-center justify-center h-[50vh] gap-4">
                <h2 className="text-xl font-semibold text-red-600">{error || "Klien tidak ditemukan"}</h2>
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 pb-6 border-b border-gray-200">
                ...
                <div className="flex items-center gap-2">
                    <div className="bg-gray-50 border border-gray-200 rounded-md px-4 py-2 flex flex-col items-end">
                        <span className="text-[10px] font-bold text-gray-400 uppercase">Agen Penanggung Jawab</span>
                        <span className="text-sm font-semibold text-black">{client.agent_name || "—"}</span>
                    </div>
                </div>
            </div>
