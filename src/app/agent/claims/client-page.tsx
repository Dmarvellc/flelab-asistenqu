        ...
        <div className="flex flex-col gap-8 animate-in fade-in duration-500 w-full">
            ...
            <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4 pb-6 border-b border-gray-200">
                <div className="min-w-0">
                    ...
                    <h1 className="text-2xl sm:text-3xl font-bold tracking-tight text-black">Daftar Klaim</h1>
                ...
                <div className="flex flex-wrap gap-2 shrink-0">
                    {agencyTemplateUrl ? (
                        <a href={agencyTemplateUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center justify-center bg-white border border-gray-200 hover:bg-gray-50 text-black text-sm font-semibold h-10 px-4 rounded-md transition-all shadow-sm hover:shadow-md gap-2">
                            <DownloadCloud className="h-4 w-4 text-gray-500" />
                            Unduh Form
                        </a>
                    ) : (
                        <button disabled className="bg-gray-50 border border-gray-200 text-gray-400 text-sm font-semibold h-10 px-4 rounded-md flex items-center gap-2 cursor-not-allowed" title="Belum diupload oleh Admin Agency">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
                <div className="bg-white rounded-lg border border-gray-200 p-8 shadow-sm hover:shadow-lg transition-all duration-300">
                    <p className="text-4xl font-bold tabular-nums text-black tracking-tight">{claims.length}</p>
                    <p className="text-[15px] font-medium text-gray-500 mt-2">Total Klaim</p>
                </div>
                <div className="bg-white rounded-lg border border-gray-200 p-8 shadow-sm hover:shadow-lg transition-all duration-300">
                    <p className="text-4xl font-bold tabular-nums text-black tracking-tight">{statusCounts['APPROVED'] || 0}</p>
                    <p className="text-[15px] font-medium text-gray-500 mt-2">Disetujui</p>
                </div>
                <div className="bg-white rounded-lg border border-gray-200 p-8 shadow-sm hover:shadow-lg transition-all duration-300 col-span-2 md:col-span-1">
                    <p className="text-4xl font-bold tabular-nums text-black tracking-tight">
            ...
            <div className="bg-white rounded-lg border border-gray-200 overflow-hidden shadow-sm flex flex-col">
                {/* Toolbar */}
                <div className="flex flex-col lg:flex-row lg:items-center gap-4 sm:gap-6 p-4 sm:p-8 border-b border-gray-50 bg-white">
                    <div className="relative flex-1 max-w-md">
                        <Search className="absolute left-4 top-1/2 -translate-y-1/2 h-5 w-5 text-gray-400" />
                        <Input
                            type="search"
                            placeholder="Cari nasabah, RS, atau penyakit..."
                            className="pl-12 h-12 bg-white border-gray-200 text-[15px] rounded-md focus:bg-white focus:border-gray-300 focus:ring-4 focus:ring-gray-900/5 transition-all w-full shadow-sm"
                            value={search}
                    ...
                    <div className="flex items-center gap-2 flex-wrap">
                        <button
                            ...
                            className={cn(
                                "text-[14px] font-semibold px-5 py-2.5 rounded-md transition-all duration-200 shadow-sm border",
                                !activeFilter ? "bg-black text-white border-transparent" : "bg-white text-gray-600 border-gray-200 hover:bg-gray-50"
                            )}
                        >
                            Semua
                        </button>
                        {filters.map(f => (
                            statusCounts[f] ? (
                                <button
                                    ...
                                    className={cn(
                                        "text-[14px] font-semibold px-5 py-2.5 rounded-md transition-all duration-200 shadow-sm border",
                                        activeFilter === f ? "bg-black text-white border-transparent" : "bg-white text-gray-600 border-gray-200 hover:bg-gray-50"
                                    )}
                                >
                ...
                <div className="p-4 sm:p-8">
                    {loading ? (
                        <div className="flex flex-col items-center justify-center py-32 gap-6">
                            <div className="h-12 w-12 rounded-full border-[3px] border-gray-200 border-t-gray-900 animate-spin" />
                            <p className="text-base font-medium text-gray-400">Memuat data klaim...</p>
                        </div>
                    ) : filteredClaims.length === 0 ? (
                        <div className="flex flex-col items-center justify-center py-32 text-center text-gray-500 bg-white rounded-[24px] border border-gray-200/50 m-4">
                            <div className="h-20 w-20 rounded-[24px] bg-white border border-gray-200 shadow-sm flex items-center justify-center mb-6">
                                <FileText className="h-8 w-8 text-gray-300" />
                            </div>
                            <p className="text-xl font-bold text-black mb-2">Tidak ada klaim</p>
                        ...
                        <div className="grid gap-4">
                            {filteredClaims.map((claim) => {
                                ...
                                return (
                                    <div
                                        key={claim.claim_id}
                                        className="group flex flex-col lg:flex-row lg:items-center justify-between p-6 bg-white border border-gray-200 hover:border-gray-200 hover:shadow-md rounded-md transition-all duration-300"
                                    >
                                        <div className="flex flex-col gap-3 flex-1 min-w-0 pr-6">
                                            <div className="flex items-center justify-between lg:justify-start gap-4 flex-wrap w-full">
                                                <div className="flex flex-col">
                                                    <h3 className="font-bold text-black text-lg">{claim.client_name}</h3>
                                                    <span className="text-[13px] font-semibold text-gray-400 mt-0.5">
                                                        #{claim.claim_number || claim.claim_id.slice(0, 8)}
                                                    </span>
                                                </div>

                                                <div className="flex items-center gap-2 text-[13px] font-bold px-3 py-1.5 rounded-lg border border-gray-200 bg-gray-50 shadow-sm w-max lg:ml-auto">
                                                    {cfg.icon}
                                                    <span className={cn(
                                            ...
                                            <div className="flex flex-wrap items-center gap-x-4 gap-y-2 text-[14px] text-gray-500 mt-2">
                                                ...
                                                <span className="text-gray-300 hidden sm:inline">•</span>
                                                <div className="flex items-center gap-1.5">
                                                    <span className="font-bold text-black bg-gray-50 px-2 py-0.5 rounded-md border border-gray-200 shadow-sm">
                                        ...
                                        <Link
                                            ...
                                            className="mt-6 lg:mt-0 lg:ml-6 shrink-0 inline-flex items-center justify-center gap-2 text-[14px] font-bold text-black border border-gray-200 hover:border-gray-900 bg-white hover:bg-gray-50 px-6 py-3.5 rounded-md shadow-sm hover:shadow-md transition-all duration-300 w-full lg:w-max group-hover:-translate-y-0.5"
                                        >
                                            Lihat Detail
