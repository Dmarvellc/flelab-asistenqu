...
import {
  ...
  ArrowRight, TrendingUp, Hospital, Filter, Video
} from "lucide-react"
...
function StatCard({ icon: Icon, label, value, sub, accent }: {
  ...
  return (
    <div className="flex items-center gap-4 bg-white border border-gray-100 rounded-2xl p-5 hover:shadow-sm transition-shadow">
      <div className={`h-12 w-12 rounded-xl flex items-center justify-center shrink-0 ${accent || "bg-gray-50 text-gray-600"}`}>
...
// ─── Doctor Card ────────────────────────────────────────────
function DoctorCard({ doctor }: { doctor: NetworkDoctor }) {
  ...
  return (
    <div className="group bg-white border border-gray-100 rounded-2xl p-5 flex flex-col gap-3 hover:border-gray-300 hover:shadow-sm transition-all duration-200">
      <div className="flex items-start justify-between gap-2">
        <div className="min-w-0">
      ...
      <div className="flex items-start gap-2 mt-auto">
        ...
        <div className="min-w-0">
          ...
          <p className="text-[11px] text-gray-400 mt-0.5">{doctor.hospital_location}</p>
        </div>
      </div>
...
// ─── Tab type ───────────────────────────────────────────────
type ActiveTab = "hospitals" | "doctors"
  ...
  return (
    <div className="flex flex-col gap-8 animate-in fade-in duration-500">
      ...
      <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 p-6 sm:p-10">
        ...
        <div className="relative z-10">
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-[11px] font-bold tracking-widest uppercase text-white/60">Network Intelligence</span>
          </div>
          <h1 className="text-2xl sm:text-4xl font-bold text-white tracking-tight">
            Jaringan Rumah Sakit
          </h1>
          <p className="text-sm sm:text-base text-white/60 mt-2 max-w-xl">
            Akses data dokter, spesialisasi, teknologi, dan akreditasi dari rumah sakit terkemuka di Asia Tenggara.
          </p>
        </div>
      </div>
      ...
      {stats && stats.countries.length > 0 && (
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          {stats.countries.map(c => (
            <button
              key={c.country}
              onClick={() => { setCountry(prev => prev === c.country ? "" : c.country); setActiveTab("hospitals") }}
      ...
      <div className="bg-white rounded-3xl border border-gray-100 overflow-hidden shadow-sm">
        ...
        {/* Search + Filters */}
        <div className="flex flex-col gap-4 p-5 sm:p-8 border-b border-gray-50 bg-gray-50/30">
          <div className="relative w-full max-w-2xl">
            ...
            <input
              ...
              placeholder={activeTab === "hospitals" ? "Cari nama rumah sakit, kota, atau teknologi..." : "Cari nama dokter, spesialisasi, atau kondisi..."}
              ...
              {SPECIALIZATIONS.map(spec => {
                ...
                return (
                  <button
                    ...
                    className={`flex items-center gap-2 whitespace-nowrap text-[13px] px-4 py-2.5 rounded-xl border shadow-sm font-semibold shrink-0 transition-all duration-200 ${
            ...
            {COUNTRIES.map(c => {
              ...
              return (
                <button
                  ...
                  className={`whitespace-nowrap text-[13px] px-4 py-2.5 rounded-xl border shadow-sm font-semibold shrink-0 transition-all duration-200 ${
        ...
        <div className="p-4 sm:p-8">
          {loading && currentItems.length === 0 ? (
            <div className={`grid gap-4 sm:gap-6 ${activeTab === "hospitals" ? "grid-cols-1 sm:grid-cols-2 lg:grid-cols-3" : "grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4"}`}>
              {Array.from({ length: 6 }).map((_, i) => <CardSkeleton key={i} />)}
            </div>
          ) : currentItems.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-20 text-center bg-gray-50/50 rounded-2xl border border-gray-100/50 min-h-[300px]">
              <div className="h-16 w-16 rounded-2xl bg-white border border-gray-100 flex items-center justify-center mb-5 shadow-sm">
                <Search className="h-7 w-7 text-gray-300" />
              </div>
              <p className="font-bold text-gray-900 text-lg mb-2">Tidak ditemukan</p>
              <p className="text-sm text-gray-500 max-w-sm">Coba ubah kata kunci atau filter untuk menemukan data yang sesuai.</p>
              {hasFilters && (
                <button onClick={clearFilters} className="mt-5 text-[13px] bg-white border border-gray-200 text-gray-700 px-5 py-2.5 rounded-xl font-semibold hover:bg-gray-50 transition-all shadow-sm">
                  Hapus semua filter
                </button>
              )}
            </div>
