...
const DRAFT_KEY = "hospital_claim_draft_v1";
...
export default function NewClaimPage() {
    ...
    // Data Sources
    const [hospitalLabel, setHospitalLabel] = useState<string>("");
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const [ctxRes, diseasesRes] = await Promise.all([
                    fetch("/api/hospital/session-context"),
                    fetch("/api/diseases"),
                ]);

                if (ctxRes.ok) {
                    const data = await ctxRes.json();
                    if (data.hospital?.name) setHospitalLabel(data.hospital.name as string);
                }
    ...
    useEffect(() => {
        ...
        const t = setTimeout(async () => {
            ...
            try {
                const res = await fetch(`/api/hospital/claim-clients?q=${encodeURIComponent(q)}`);
        ...
        try {
            ...
            if (res.ok) {
                ...
                toast({
                    ...
                    description: errPayload.error || "Terjadi kesalahan saat menyimpan data.",
                    variant: "destructive"
                });
            }
        } catch (error) {
    ...
    const getClientName = () => pickedClientName || clients.find(c => c.client_id === formData.client_id)?.full_name || "-";
    const getHospitalDisplay = () => hospitalLabel || "Rumah sakit Anda";
            ...
            <div className="flex flex-1 min-h-0">
                ...
                <div className="w-60 shrink-0 border-r border-border/60 pr-6 flex flex-col pt-1">
                    <div className="mb-8">
                        ...
                        <p className="text-xs text-muted-foreground mt-1">Buka klaim dari rumah sakit; agen nasabah akan melihat klaim ini.</p>
                    </div>
                        ...
                        >
                            ...
                            {currentStep === 1 && (
                                <div className="max-w-lg space-y-6">
                                    <div className="mb-6">
                                        <h2 className="text-lg font-semibold">Cari Nasabah</h2>
                                        <p className="text-sm text-muted-foreground mt-1">
                                            Ketik nama atau NIK (minimal 3 huruf). Hanya nasabah yang sudah punya agen penanggung.
                                        </p>
                                    </div>

                                    <div className="space-y-2">
                                        <Label className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Cari nasabah</Label>
                                        <Input
                                            value={clientSearchInput}
                                            onChange={(e) => setClientSearchInput(e.target.value)}
                                            placeholder="Nama lengkap atau NIK…"
                                            className={cn("h-10 rounded-lg bg-background", fieldErrors.client_id && "border-red-500")}
                                        />
                                        {loadingClients && (
                                        ...
                                        {clientSearchInput.trim().length > 0 && clientSearchInput.trim().length < 3 && (
                                            <p className="text-xs text-muted-foreground">Ketik setidaknya 3 karakter.</p>
                                        )}
                                        {!loadingClients && clientSearchInput.trim().length >= 3 && clients.length === 0 && (
                                            <p className="text-xs text-muted-foreground">Tidak ada nasabah yang cocok.</p>
                                        )}
                                        {clients.length > 0 && (
                                            <div className="rounded-md border border-border/60 bg-muted/15 max-h-56 overflow-y-auto divide-y divide-border/50">
                                                {clients.map((c) => (
                                                    <button
                                                        key={c.client_id}
                                                        type="button"
                                                        className={cn(
                                                            ...
                                                            formData.client_id === c.client_id && "bg-muted"
                                                        )}
                                                        onClick={() => {
                                                            setFormData({ ...formData, client_id: c.client_id });
                                                            setPickedClientName(c.full_name);
                                                            setClientSearchInput(c.full_name);
                                        ...
                                        {formData.client_id && (
                                            <div className="rounded-md bg-emerald-50 border border-emerald-200 px-3 py-2 text-sm text-emerald-900">
                                                Terpilih: <span className="font-semibold">{pickedClientName || getClientName()}</span>
                                            </div>
                                        )}
                                ...
                                <div className="space-y-6 pb-4">
                                    ...
                                    <div className="rounded-md border border-border/60 p-5 space-y-4 bg-muted/20">
                                        <h3 className="text-xs font-semibold uppercase tracking-wider text-muted-foreground flex items-center gap-2">
                                            <Building2 className="w-3.5 h-3.5" /> Rumah sakit Anda
                                        </h3>
                                        <p className="text-sm text-muted-foreground">
                                            Klaim ini otomatis diajukan untuk <span className="font-semibold text-foreground">{getHospitalDisplay()}</span>.
                                        </p>
                                    </div>
                                    ...
                                    <div className="rounded-md border border-border/60 p-5 space-y-4 bg-muted/20">
                                        ...
                                        <div className="max-w-xs space-y-1.5">
                                            <Label className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Tanggal masuk RS / kejadian</Label>
                                            <Input
                                                type="date"
                                            ...
                                            <div className="space-y-4">
                                                ...
                                                <div className="space-y-3">
                                                    <div>
                                                        ...
                                                        <span className="font-medium">{getHospitalDisplay()}</span>
                                                    </div>
                                                    <div>
