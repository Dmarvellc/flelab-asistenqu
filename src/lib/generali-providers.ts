// Generali insurance network provider directory.
// Extracted from /agent/network/overseas page so the data can be merged into the unified marketplace.

export interface GeneraliProvider {
  no: number
  type: string
  name: string
  address: string
...
export function extractState(address: string, country: string): string {
...
export function extractNetwork(name: string): string {
...
const RAW_DATA: Omit<GeneraliProvider, "state" | "city" | "network">[] = [
...
export const GENERALI_PROVIDERS: GeneraliProvider[] = RAW_DATA.map((r) => ({
  ...r,
